...
# ── COLOR ────────────────────────────────────────────────────
colors:
  linen:  "#E4D2C4"   # canvas — warm putty/oatmeal cloth, the page AND the negative space
  ink:    "#1F1A14"   # ink/structure — warm near-black; ALL type, thin rules, fine accents
  blue:   "#044D99"   # accent-1 — primary cobalt block
  green:  "#04B24F"   # accent-2 — primary grass-green block
  red:    "#F61B27"   # accent-3 — primary red block
  # Rule: the three primaries are a TRIAD — use them loudly together as graphic blocks, OR
  # pick ONE as the scene's hero accent. Ink for ALL type. Dark text on linen / on light fills.
  # The primaries are SHAPE colors; keep small text off them (place labels on linen, never on a block).
...
**Linen** (`#E4D2C4`) is the page — a warm oatmeal cloth that also reads as the *negative*
space around each block. **Ink** (`#1F1A14`, a warm near-black) is the whole type system
plus the occasional 2–3px structural rule. The three **primaries — blue `#044D99`, green
`#04B24F`, red `#F61B27`** — are the flat color blocks and discs.

Use the primaries one of two ways, never muddied:

- **Triad mode (maximalist, on-brand):** all three blocks together as the hero composition (like the cover). Loud but balanced because they're flat and separated by linen.
- **Single-accent mode:** one primary as the scene's hero block, the rest of the board ink-on-linen.

The primaries are **shape colors**. Keep small labels on the linen, never set small text on a
block fill (low-contrast and noisy). Big display numerals in ink may sit *beside* a block, in
their own zone.
